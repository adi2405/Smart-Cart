...
@app.route('/predict_birch', methods=['POST'])
def predict_birch():
    if 'file' not in request.files:
        return jsonify({'error': 'No file part'}), 400
    ...
    features = dataset[['Unit price', 'Quantity', 'Tax 5%', 'Total', 'cogs', 'gross income', 'Rating']]
    ...
    birch_model = Birch(n_clusters=None)  # Letting BIRCH decide the optimal number of clusters
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(dataset['Unit price'], dataset['Total'], c=dataset['Cluster'], cmap='viridis')
    plt.xlabel('Unit Price')
    plt.ylabel('Total')
    plt.title('BIRCH Clustering of Supermarket Sales Data')
    plt.colorbar(label='Cluster')

    # Save the cluster plot image
    cluster_img_path = 'static/cluster_plot_birch.png'
    plt.savefig(cluster_img_path)
    plt.close()

    # Return the result along with the image path
    return jsonify({
        'data': dataset.to_dict(orient='records'),
        'cluster_image': cluster_img_path
    }), 200

@app.route('/predict_dbscan', methods=['POST'])
def predict_dbscan():
    if 'file' not in request.files:
        return jsonify({'error': 'No file part'}), 400
    ...
    plt.xlabel('Unit Price')
    plt.ylabel('Total')
    plt.title('DBSCAN Clustering of Supermarket Sales Data')
    plt.colorbar(label='Cluster')

    # Save the cluster plot image
    cluster_img_path = 'static/cluster_plot_dbscan.png'
    plt.savefig(cluster_img_path)
    plt.close()
